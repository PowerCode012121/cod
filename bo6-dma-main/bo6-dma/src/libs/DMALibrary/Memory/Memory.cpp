...
bool Memory::Init(std::string process_name, bool memMap, bool debug, bool manual_refresh)
{
	if (!DMA_INITIALIZED)
	{
		LOG("inizializing...\n");
...
bool Memory::FixCr3()
{
	...
	while (true)
	{
		...
		std::this_thread::sleep_for(std::chrono::milliseconds(100)); // makes more consistent i think? but slower
	}
	...
	bool result = VMMDLL_VfsListU(this->vHandle, const_cast<LPSTR>("\\misc\\procinfo\\"), &VfsFileList);
	if (!result)
		return false;
	...
	for (size_t i = 0; i < possible_dtbs.size(); i++)
	{
		...
		VMMDLL_ConfigSet(this->vHandle, VMMDLL_OPT_PROCESS_DTB | current_process.PID, dtb); // VMMDLL_OPT_PROCESS_DTB_FAST_LOWINTEGRITY is without slow refresh
		...
		if (result)
		{
			...
			LOG("[+] Patched DTB (Took %ims | %i/%i at %llx)\n", diff, i + 1, possible_dtbs.size(), dtb);
			return true;
		}
	}

	LOG("[-] Failed to patch module out of %i possible dtbs\n", possible_dtbs.size());
	return false;
}

bool Memory::NewFixCr3()
{
